...
TARGET_READER_PORT = 5603
TARGET_SENDER_PORT = 5604
...
def main():
    futures = []
    queue = Queue()  
    socket_reader = MySocket('READER_SOCKET_CLIENT1', MY_HOST, READER_PORT)
    socket_sender = MySocket('SENDER_SOCKET_CLIENT1', MY_HOST, SENDER_PORT)
    ...
    reader_process = Process(target= bot_send_messages, args =(socket_sender, 
                            TARGET_HOST, TARGET_READER_PORT,
                                       message_dict, queue))
    queue.put(b'Start conversation')
